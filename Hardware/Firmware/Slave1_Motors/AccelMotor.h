...
public:
    ...
    // controla el motor. Llame con la mayor frecuencia posible (dentro de un temporizador con período dt)
    // toma la posición actual del eje del motor (según el codificador)
    // devuelve verdadero si el motor todavía se mueve hacia el objetivo
    bool tick(long pos);							
    ...
    // establecer y obtener la velocidad objetivo en tics del codificador / segundo y grados / segundo
    ...
    //establecer la zona de parada del motor para el modo de estabilización de posición (por defecto 8)
    void setStopZone(int zone);
    
    // establezca los límites de pasos / grados fuera de los cuales el motor se desactivará por seguridad. Si hay ceros, no hay límite (predeterminado)
